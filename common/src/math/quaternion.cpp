...
const Quaternion Quaternion::IDENTITY(1,0,0,0);   // w, x, y, z


Quaternion::Quaternion(const Radian& rXAngle, const Radian& rYAngle, const Radian& rZAngle)
{
    Quaternion qx, qy, qz;
    qx.FromAngleAxis(rXAngle, Vector3::UNIT_X);
    qy.FromAngleAxis(rYAngle, Vector3::UNIT_Y);
    qz.FromAngleAxis(rZAngle, Vector3::UNIT_Z);

    // Quaternion for rotation is yaw * pitch * roll (y * x * z)
    *this = qy * qx * qz;
}
...
Quaternion Quaternion::Exp () const
{
    ...
    Radian fAngle ( sqrtf(x*x+y*y+z*z) );
...
{
    float fCos = Dot(rhs);
    Radian angle = acosf(fCos);    
    return (fabs(angle) <= tolerance) || fabs(angle - M_PI) <= tolerance;
}
                              ...
                              const Quaternion& rkQ, bool shortestPath)
{
    ...
    if (fabs(fCos) < 1 - msEpsilon)
    {
        ...
        Radian fAngle = atan2f(fSin, fCos);
...
{
    float fCos = rkP.Dot(rkQ);
    Radian fAngle ( acosf(fCos) );
    
    if ( fabs(fAngle) < msEpsilon )
        return rkP;
    
    float fSin = sinf(fAngle);
    Radian fPhase ( M_PI * iExtraSpins * fT );
    float fInvSin = 1.0f/fSin;
